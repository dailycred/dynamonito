...
/**
 * A partial implementation of {@link AmazonDynamoDB}. This intercepter
 * intercepts writes from the high level mapper and captures the JSON used in
 * DynamoDB's wire transfer protocol.
 * 
 */
public class AmazonDynamoDBIntercepter implements AmazonDynamoDB {
	...
	public UpdateItemResult updateItem(UpdateItemRequest updateItemRequest)
		 ...
		 */
		Map<String, AttributeValueUpdate> attributeUpdates = updateItemRequest
				.getAttributeUpdates();
		for (Entry<String, AttributeValueUpdate> en : attributeUpdates
				.entrySet()) {
			String action = en.getValue().getAction();
			if (action.equals("PUT")) {
				AttributeValue av = en.getValue().getValue();
				cachedAttributes.put(en.getKey(),
						Util.getJsonObjFromAttributeValue(av));
			}
		}
		return null;
	}
